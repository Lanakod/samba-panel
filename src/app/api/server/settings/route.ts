...
export async function GET() {
  try {
    ...
    if (!global) {
      return Response.json({ status: false, message: 'Global section not found', error: 'Global section not found' }, { status: 404 });
    }
    ...
    console.error(e);
    return Response.json({ status: false, message: 'Failed to load global section', error: 'Failed to load global section' }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error(e);
    return Response.json({ status: false, message: "Failed to update global section", error: 'Failed to update global section' }, { status: 500 });
  }
}
